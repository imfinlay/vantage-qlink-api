...
- `ctx.VGS_CACHE` / `ctx.LOAD_CACHE` hold the last known value per key with a timestamp; routes serve from cache when fresh (`cacheMs` param). For `/status/vgs`, `MIN_POLL_INTERVAL_MS` is a floor under whatever `cacheMs` a request sends (`cacheMs=0` bypasses it); `/dim` only uses it as the default. `/test/vsw` deletes the written switch's `VGS_CACHE`/`STATE` entries so the next status read polls.
...
**Load push.** With `VOL 1` enabled on the controller, load changes arrive unsolicited as `LO <master> <enclosure> <module> <load> <level>` (the level is the target; fades don't stream). `core/parsing.js` always stores them in `ctx.LOAD_CACHE` (via the shared `applyLoad()`, which also wakes `LOAD_AWAITERS`, same as `RLB`/`RGB`). Only when `ctx.LOAD_PUSH` is on (built-in default off; the repo's `config.js` turns it on) does `GET /dim` treat the cache as authoritative (ignoring `cacheMs`, except `cacheMs=0`), re-polling an entry older than `LOAD_PUSH_MAX_AGE_MS` as a safety net; `ensureDisconnected()` clears `LOAD_CACHE`. There is no load whitelist or startup seed: the first read of each load polls and fills the cache. `/dim` addresses must be the load's own address, not a switch pointer or scene button; station-bus dimmers (`LS`) are unsupported.

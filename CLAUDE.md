...
**Config resolution**: `config.js` at the repo **root** (tracked in git, contains this deployment's real controller IP and filesystem paths — it is not a template) is the source of truth; `src/config.js` is just a shim re-exporting it (`module.exports = require('../config')`). `core/context.js` builds `ctx`'s config-derived fields by checking `process.env.*` first, then `config.js`, then a hardcoded fallback — most timing/behavior knobs (`MIN_GAP_MS`, `PUSH_FRESH_MS`, `HB_WHITELIST_STRICT`, etc.) can be overridden per-deployment via environment variables without editing `config.js`.
...
- The Vantage line protocol (`VGS#`/`RGS#`/`VLB#`/`RLB#`/`RGB#`/`SW`/`VOS`) is documented in `README_SPLIT.md` and `README.md`'s "HTTP API" / "Protocol details" sections — check there before guessing at command formats, and check `commands.csv` for the full accepted command list.
